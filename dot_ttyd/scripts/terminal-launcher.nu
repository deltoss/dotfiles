...
let selection = null | fzf --preview $env.FZF_CUSTOM_PREVIEW --bind $"change:reload-sync\(sleep 100ms; ($es_template)\)" --phony --query "" --header="Terminal Launcher"
if ($selection | is-not-empty) {
  let folder = match ($selection | path type) {
    "dir" => $selection,
    _ => ($selection | path dirname)
  }

  let wezterm_running = (ps | where name =~ "wezterm-gui" | is-not-empty)
  if $wezterm_running {
    let pane_id = ^wezterm cli spawn --cwd $folder
    ^wezterm cli activate-pane --pane-id $pane_id
    print $"(ansi green)Spawned new tab, switch to your Wezterm terminal(ansi reset)"
  } else {
    run-detached wezterm-gui start --cwd $folder
  }
}