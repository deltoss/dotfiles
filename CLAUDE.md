...
`.chezmoidata/packages/windows.toml` defines a flat list of `[[packages.windows]]` entries, filtered by `tags`: `"all"` always installs; `"personal"`/`"work"` must match `.computerPurpose`. (`packages/linux.toml` is a stub for future Linux entries.) Installation happens in `.chezmoiscripts/windows/run_onchange_after_install-packages.nu.tmpl` — Nushell, self-elevates via `gsudo`.
